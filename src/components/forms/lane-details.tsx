...
interface CreateLaneDetailsProps {
  defaultData?: Lane
  projectId: string
}
...
const LaneDetails: React.FC<CreateLaneDetailsProps> = ({
  defaultData,
  projectId,
}) => {
  ...
  const onSubmit = async (values: z.infer<typeof LaneFormSchema>) => {
    ...
    try {
      ...
      toast('Success', {
        description: 'Saved List details',
      });

      router.refresh()
    } catch (error) {
      toast('Oppse!', {
        description: 'Could not save list details',
      });
    }
    setClose()
  }
